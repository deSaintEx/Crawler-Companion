...
public class JdbcUserDao implements UserDao {
    ...
    public User updateUser(User updatedUser) {
        ...
        String sql = "UPDATE users " +
                ...
                "WHERE user_id = ?;";
        try {
            int rowsAffected = jdbcTemplate.update(sql, updatedUser.getUsername(),
                    updatedUser.getHashedPassword(), updatedUser.getRole(), updatedUser.getId());

            if (rowsAffected == 0) {
                throw new DaoException("Zero rows affected, expected at least 1");
            } else {
                user = getUserById(updatedUser.getId());
            }
        } catch (CannotGetJdbcConnectionException e ) {
